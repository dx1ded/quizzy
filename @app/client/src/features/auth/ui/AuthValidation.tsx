...
export function AuthValidation({ error, initialErrors }: AuthValidationProps) {
  if (!error || !initialErrors) return

  return (
    <div className="absolute -right-4 top-1/2 -translate-y-1/2 translate-x-full rounded bg-white py-2 pl-2.5 pr-4 shadow before:absolute before:left-0 before:top-1/2 before:-translate-x-full before:-translate-y-1/2 before:rotate-0 before:border-y-8 before:border-r-8 before:border-solid before:border-transparent before:border-r-neutral-200">
      <ul>
        {initialErrors.map((initialError, i) =>
          ...
          (error.types?.invalid_string as string[]).includes(initialError) ? (
            <li
              key={i}
              className="flex items-center gap-1.5 text-xs [&:not(:last-child)]:mb-2">
